        ...
        if ($matchesmodid) {
            foreach ($matchesmodid as $match) {
                if (!in_array((string)$thisID, $match)) {
                    ...
                    $module = JModuleHelper::getModuleById(trim($match[1])); //$id = trim($match[1]);
